...
import argparse,zipfile,tempfile,subprocess,json,sys,hashlib,datetime,csv,shutil
...
for v in reg.get('validators',[]):
 print('START',v['id'],file=sys.stderr,flush=True)
 before=tree_hash(root);rp=out/f"{v['id']}.json";cmd=[sys.executable,str(HERE/'scripts'/v['script']),str(root),'--config',str(cfg),'--profile',str(Path(a.profile).resolve()),'--output',str(rp)];proc=subprocess.run(cmd,capture_output=True,text=True);after=tree_hash(root)
 try:r=json.loads(rp.read_text())
 except:r={'status':'FAIL','errors':[proc.stderr or proc.stdout or 'validator produced no result']}
 if before!=after:r={'status':'FAIL','errors':r.get('errors',[])+['validator mutated candidate tree']}
 ...
 print('END',v['id'],r.get('status'),file=sys.stderr,flush=True)
 if r.get('status')!='PASS':findings.append({'finding_id':f'AUTO-{len(findings)+1:03d}','severity':'P1','rule_id':v['id'],'dimension':d,'status':'open','summary':'; '.join(r.get('errors',[])[:8]),'evidence':[rp.name],'blocking':True})
