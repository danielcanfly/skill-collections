...
def main():
 ...
 for rel in ['qa/source_snapshot_validation.json','qa/external_stage_review_validation.json','qa/passage_stage_validation.json','qa/clause_stage_validation.json','qa/claim_compilation_receipt.json']:
  j=load(r/rel)
 ...
 direct=[q for q in edges if q.get('support_label')=='support' and (q.get('final_disposition') or 'retain') not in {'exclude','defer','remove'}]
 ...
 if len(direct)!=len({(q.get('claim_id'),q.get('source_id'),q.get('passage_id')) for q in direct}):e.append('duplicate active direct-support edge identity')
