#!/usr/bin/env python3
"""Deterministic lexical retrieval smoke test for an BFA production candidate.
...
def manifest(root,docs):
    ...
    return {"generated_at":"2026-07-15T10:30:00+00:00","configuration":CONFIG,"document_count":len(files),"corpus_hash":digest.hexdigest(),"files":files}
...
def main():
    ...
    for t in tests:
        ...
        passed+=int(ok)
        results.append({**t,"status":"PASS" if ok else "FAIL","query_sha256":hashlib.sha256(t["query"].encode()).hexdigest(),"corpus_sha256":corpus_hash,"ranked_ids":[d["id"] for s,d in ranked],"expected_ranks":ranks,"negative_check_passed":neg_ok,"top_k":[{"rank":i+1,"id":d["id"],"title":d["title"],"score":round(s,6),"path":d["path"]} for i,(s,d) in enumerate(ranked)]})
    out={"generated_at":"2026-07-15T10:30:00+00:00","corpus_hash":cm["corpus_hash"],"configuration":CONFIG,"scope_note":"Lexical smoke test only; not a production vector/reranker/graph benchmark.","summary":{"total":len(tests),"passed":passed,"failed":len(tests)-passed},"results":results}
