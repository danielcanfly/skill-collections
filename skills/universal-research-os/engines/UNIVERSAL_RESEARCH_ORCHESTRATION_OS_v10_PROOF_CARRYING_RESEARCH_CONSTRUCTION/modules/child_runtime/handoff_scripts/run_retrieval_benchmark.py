...
def manifest(root,docs):
    ...
    return {"generated_at":datetime.now(timezone.utc).isoformat(),"configuration":CONFIG,"document_count":len(files),"corpus_hash":digest.hexdigest(),"files":files}

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--root",default=str(Path(__file__).resolve().parents[1])); args=ap.parse_args()
    root=Path(args.root).resolve(); docs=load(root)
    ...
    for t in tests:
        ...
        passed+=int(ok)
        results.append({**t,"status":"PASS" if ok else "FAIL","expected_ranks":ranks,"negative_check_passed":neg_ok,"top_k":[{"rank":i+1,"id":d["id"],"title":d["title"],"score":round(s,6),"path":d["path"]} for i,(s,d) in enumerate(ranked)]})
    out={"generated_at":datetime.now(timezone.utc).isoformat(),"corpus_hash":cm["corpus_hash"],"configuration":CONFIG,"scope_note":"Lexical smoke test only; not a production vector/reranker/graph benchmark.","summary":{"total":len(tests),"passed":passed,"failed":len(tests)-passed},"results":results}
